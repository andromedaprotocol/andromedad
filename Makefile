...
HTTPS_GIT := github.com/andromedaprotocol/andromedad.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=andromeda \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=andromedad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/andromedad ./cmd/andromedad
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/andromedad.exe ./cmd/andromedad
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/andromedad
...
draw-deps:
	...
	@goviz -i ./cmd/andromedad -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c andromedad --local -f chains.yaml
endif
...
ictest-upgrade:
	...
	@cd interchaintest && go test -race -v -run TestProtocolForkUpgrade .
...
ictest-ibchooks:
	@echo "Running ibchooks e2e test"
	@cd interchaintest && go test -race -v -run TestIBCHooks .
...
set-testnet-configs:
	andromedad config set client chain-id localchain-1
	andromedad config set client keyring-backend test
	andromedad config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | andromedad keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | andromedad keys add acc1 --recover`
