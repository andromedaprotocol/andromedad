...
ifeq (cleveldb,$(findstring cleveldb,$(andromeda_BUILD_OPTIONS)))
...
ifeq (,$(findstring nostrip,$(andromeda_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
build: check-go-version
	go build $(BUILD_FLAGS) -o bin/andromedad ./cmd/andromedad
...
build-reproducible-generic: go.sum
	...
	DOCKER_BUILDKIT=1 $(DOCKER) build -t latest-build-$(PLATFORM) \
		...
		-f Dockerfile .
	$(DOCKER) create -ti --name $(subst /,-,latest-build-$(PLATFORM)) latest-build-$(PLATFORM) andromedad
	$(DOCKER) cp -a $(subst /,-,latest-build-$(PLATFORM)):/usr/local/bin/andromedad andromedad_$(subst /,_,$(PLATFORM))
	sha256sum andromedad_$(subst /,_,$(PLATFORM)) >> ./andromedad_sha256.txt

# Add check to make sure we are using the proper Go version before proceeding with anything
check-go-version:
	@if ! go version | grep -q "go$(GO_VERSION)"; then \
		echo "\033[0;31mERROR:\033[0m Go version $(GO_VERSION) is required for compiling andromedad. It looks like you are using" "$(shell go version) \nThere are potential consensus-breaking changes that can occur when running binaries compiled with different versions of Go. Please download Go version $(GO_VERSION) and retry. Thank you!"; \
		exit 1; \
	fi
