# docker build . -t andromedaprotocol/andromedad:latest
# docker run --rm -it andromedaprotocol/andromedad:latest /bin/sh
...
RUN echo "Ensuring binary is statically linked ..." \
  && (file /code/build/andromedad | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.15

COPY --from=go-builder /code/build/andromedad /usr/bin/andromedad
...
CMD ["/usr/bin/andromedad", "version"]