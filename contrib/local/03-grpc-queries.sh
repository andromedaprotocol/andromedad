...
PROTO_ANDROMEDAD="$DIR/../../proto"
PROTO_ANDROMEDAD_QUERY="$PROTO_ANDROMEDAD/cosmwasm/wasm/v1/query.proto"

echo "### List all codes"
RESP=$(grpcurl -plaintext -import-path "$PROTO_THRD" -import-path "$PROTO_ANDROMEDAD" -proto "$PROTO_ANDROMEDAD_QUERY" \
...
grpcurl -plaintext -import-path "$PROTO_THRD" -import-path "$PROTO_ANDROMEDAD" -proto "$PROTO_ANDROMEDAD_QUERY" \
